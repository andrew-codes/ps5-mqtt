...
function* updateAccount({ payload: account }: UpdateAccountAction) {
  ...
  if (account.activity !== undefined) {
    ...
    bestMatch = devices.find(
      (d) => d.status === "AWAKE" && d.type === account.activity.launchPlatform,
    )
    if (bestMatch !== undefined) {
      // if there already is an activity on that device add the player to the active player list
      if (
        bestMatch.activity !== undefined &&
        !bestMatch.activity.activePlayers.includes(account.accountName)
      ) {
  ...
  for (const device of devices.filter(
    (d) =>
      d !== bestMatch &&
      d.activity?.activePlayers?.includes(account.accountName),
  )) {
    // remove player from player list
    if (device.activity.activePlayers.length > 1) {
      const accountIndex = device.activity.activePlayers.indexOf(
        account.accountName,
      )
